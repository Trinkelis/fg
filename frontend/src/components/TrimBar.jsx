import { useRef, useEffect } from 'react';
import useStore from '../store/useStore.js';

export default function TrimBar({ videoRef }) {
  const { operations, setOperation, currentTime, duration, setCurrentTime } = useStore();
  const barRef   = useRef();
  const dragging = useRef(null); // 'start' | 'end' | 'head'

  const params = operations.trim?.params || {};
  const start  = params.start ?? 0;
  const end    = params.end   ?? duration;

  const pct = t => duration > 0 ? `${(t / duration) * 100}%` : '0%';

  function getT(e) {
    const rect = barRef.current.getBoundingClientRect();
    const f    = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
    return f * duration;
  }

  function onBarDown(e) {
    // Clicking the bar itself seeks playhead
    dragging.current = 'head';
    const t = getT(e);
    setCurrentTime(t);
    if (videoRef?.current) videoRef.current.currentTime = t;
  }

  useEffect(() => {
    function onMove(e) {
      const d = dragging.current;
      if (!d) return;
      const t = getT(e);
      if (d === 'head') {
        setCurrentTime(t);
        if (videoRef?.current) videoRef.current.currentTime = t;
      } else if (d === 'start') {
        const ns = Math.max(0, Math.min(t, (end ?? duration) - 0.1));
        setOperation('trim', { start: ns, end: end ?? duration });
      } else if (d === 'end') {
        const ne = Math.min(duration, Math.max(t, start + 0.1));
        setOperation('trim', { start, end: ne });
      }
    }
    function onUp() { dragging.current = null; }
    window.addEventListener('mousemove', onMove);
    window.addEventListener('mouseup',   onUp);
    return () => {
      window.removeEventListener('mousemove', onMove);
      window.removeEventListener('mouseup',   onUp);
    };
  }, [start, end, duration, setCurrentTime, setOperation, videoRef]);

  return (
    <div className="trim-wrap">
      <div className="trim-labels">
        <span>Trim: {fmtS(start)} → {fmtS(end ?? duration)}</span>
        <span>Selection: {fmtS((end ?? duration) - start)}</span>
      </div>
      <div className="trim-track" ref={barRef} onMouseDown={onBarDown}>
        <div className="trim-sel" style={{ left:pct(start), width:pct((end ?? duration) - start) }} />
        {/* Start handle */}
        <div className="trim-h" style={{ left:pct(start) }}
          onMouseDown={e => { e.stopPropagation(); dragging.current = 'start'; }}
          title={`Start: ${fmtS(start)}`} />
        {/* End handle */}
        <div className="trim-h" style={{ left:pct(end ?? duration) }}
          onMouseDown={e => { e.stopPropagation(); dragging.current = 'end'; }}
          title={`End: ${fmtS(end ?? duration)}`} />
        {/* Playhead */}
        <div className="trim-ph" style={{ left:pct(currentTime) }} />
      </div>
    </div>
  );
}

function fmtS(s) {
  if (s == null || isNaN(s)) return '0:00.0';
  const m  = Math.floor(s / 60);
  const sc = (s % 60).toFixed(1);
  return `${m}:${sc.padStart(4, '0')}`;
}
